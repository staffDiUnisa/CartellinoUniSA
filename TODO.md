...
| [#8 Errore visualizzazione Saldo ore del mese](https://github.com/staffDiUnisa/CartellinoUniSA/issues/8) | URG VER | M | Alto | Bug in `_sezione_saldo` (`cartellino/tui/screens/dashboard.py` e `cartellino/gui/screens/dashboard.py`, entrambe basate su `somma_ore_per_codici`/`cartellino/ore_helpers.py`): tutti i codici in `dashboard_balance_codes` (default `CRE, OE-DIU, SCN`) vengono sommati con lo stesso segno, ma `estrai_ore_minuti` legge solo il valore assoluto da "Voci Base" — `SCN` è uno scostamento negativo e andrebbe sottratto, non addizionato, quindi il totale risulta sempre ≥ 0 e il ramo "segno negativo" già presente nel codice (`segno = "-" if totale_minuti < 0 else ""`) non scatta mai. Serve introdurre una convenzione per marcare quali codici sono sottrattivi (nuovo campo config, es. lista dedicata, esposto nelle Impostazioni sia TUI che GUI) oltre alla correzione del calcolo in entrambi i frontend. Richiede inoltre la colorazione del valore (Blu/Verde/Rosso per zero/positivo/negativo): banale in TUI (markup Rich, già usato altrove), da aggiungere via stylesheet dinamico su `QLabel` in GUI. Nessun workaround per l'utente (il dato mostrato è semplicemente sbagliato), su una funzionalità già rilasciata — **URG**. |
| [#9 Miglioramenti UI](https://github.com/staffDiUnisa/CartellinoUniSA/issues/9) |   VER   | L | Medio | Due richieste distinte: (1) icone significative sui pulsanti di tutte le schermate TUI e GUI, con colori differenziati per funzione — tocca praticamente ogni `Button`/`QPushButton` esistente, serve scegliere un font/set di icone compatibile sia col rendering a celle di Textual sia con Qt (es. icone Unicode/Nerd Font per la TUI, un font-icona tipo Material/Font Awesome per la GUI) e una convenzione di colore coerente con `app.tcss`; (2) pulsante "Esci" nella GUI con conferma prima della chiusura (`QMessageBox` su `closeEvent`/pulsante dedicato in `MainWindow`) — la TUI ha già `Ctrl+Q`/uscita da tastiera, la GUI ne è priva. Rischio principale: la richiesta (1) è ampia in superficie (molte schermate) più che in difficoltà tecnica singola. |
